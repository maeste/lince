...
> **macOS validation status.** The Seatbelt profile generation and the credential-proxy allowlist are covered by host-side tests that run on Linux; the generated SBPL forms have **not yet been validated on real macOS hardware**. Until the manual checklist in `sandbox/to-be-validated.md` is executed on a Mac, treat the behavioral claims in this section (loopback-only containment, DNS resolution failing for arbitrary hosts under `(deny network*)`) as expected-but-unverified. One known divergence from bwrap to keep in mind: SBPL `(deny network*)` also denies **unix-domain sockets** (Seatbelt models them as network operations, and the loopback allows only cover TCP/IP), whereas bwrap's network namespace leaves unix sockets working — that is how the Linux paranoid proxy bridge over `/run/lince-proxy.sock` operates. On macOS at paranoid, loopback TCP is the only in-sandbox transport.
